...
interface TtsPlaybackProps { text: string; label?: string; compact?: boolean }
...
export function TtsPlayback({ text, label = 'Listen', compact }: TtsPlaybackProps) {
  ...
  const ttsMutation = useMutation({
    mutationFn: async (txt: string) => {
      ...
      return api<{ audio: string }>('/api/ai/tts', { method: 'POST', body: JSON.stringify({ text: txt }) })
    },
  ...
  const handleClick = () => {
    if (isPlaying && audioRef.current) { audioRef.current.pause(); audioRef.current = null; setIsPlaying(false); return }
    if (!text.trim()) return
  ...
  return (
    <button type="button" onClick={handleClick} disabled={disabled}
      className={cn('inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition',
        isPlaying ? 'border-accent-brand/40 bg-accent-brand/10 text-accent-brand' : 'border-hairline bg-card-surface text-secondary-recall hover:text-primary-recall',
        disabled && 'cursor-not-allowed opacity-50')}
      aria-label={isPlaying ? 'Stop audio' : label} title={isPlaying ? 'Stop audio' : label}>
      {isLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : isPlaying ? <Square className="h-3.5 w-3.5 fill-current" /> : <Volume2 className="h-3.5 w-3.5" />}
