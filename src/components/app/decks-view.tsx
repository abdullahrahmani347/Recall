...
export function DecksView() {
  ...
  const { openDeck, startReview } = useAppStore()
  const [showCreate, setShowCreate] = useState(false)
  const [newName, setNewName] = useState('')
  const [newDesc, setNewDesc] = useState('')
  const [newColor, setNewColor] = useState('#34E7A8')

  const { data } = useQuery<{ decks: Deck[] }>({
    queryKey: ['decks'],
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-8 pt-6 sm:px-6">
      <header className="mb-6 flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-recall">Flashcards</p>
          <h1 className="font-display text-2xl font-semibold tracking-tight">Decks</h1>
        </div>
        <Button
        ...
        >
          ...
          >
            ...
            <Button
              ...
              className="mt-6 w-full bg-accent-brand text-void hover:bg-accent-brand/90"
            >
