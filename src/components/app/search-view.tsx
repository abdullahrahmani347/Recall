...
export function SearchView() {
  ...
  // Semantic search (Phase 2) — TF-IDF cosine similarity over notes
    ...
    <div className="mx-auto max-w-3xl px-4 pb-8 pt-6 sm:px-6">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className={`ml-auto flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs transition ${
            semantic
      ...
      {!debouncedQ ? (
        ...
        <p className="text-sm text-muted-recall">{semantic ? 'Computing similarity…' : 'Searching…'}</p>
      ) : semantic ? (
        semanticResults.length === 0 ? (
          ...
          <section aria-labelledby="semantic-results">
            <h2 id="semantic-results" className="mb-2 flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-muted-recall">
              ...
              {semanticResults.map((note) => (
                <li key={note.id}>
                  <button
                    ...
                    className="w-full rounded-xl border border-hairline bg-card-surface p-4 text-left transition hover:border-accent-warm/40"
                  >
